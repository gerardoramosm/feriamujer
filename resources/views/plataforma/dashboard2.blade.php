@extends('layouts.appFV',$encuesta==0 ? ['menu'=>$menu,'link'=>$link] : ['menu'=>$menu,'link'=>$link,'slider'=>true])
      ...
      <div class="panel panel-default ">
        <div class="panel-body p-3 ">
          <h2 style="text-align: center">
            <img src="{{file_exists(dirname(base_path()).'/expositores/public/images/logos/'.Auth::user()->id_empresa.'.jpg') ? url('https://expositores.feicobol.com.bo/images/logos/'.Auth::user()->id_empresa.'.jpg?1') : url('https://expositores.feicobol.com.bo/images/logos/suba_su_logo.png')}}" style="width: 30%">
            Bienvenido a la página principal de {{Auth::user()->nombre_empresa}}</h2>
          <div class="row">
            <div class="col-md-8">
              <div class="row">
                <div class="col-md-6">
...
<section class="ftco-section">
  ...
  <div class="container">
    ...
    <form class="" id="my_formDATOS" method="POST" action="{{ route('guardar-empresa') }}" enctype="multipart/form-data">
      <input type="hidden" name="solicitud" value="{{$sol->id_solicitud}}">
        {{ csrf_field() }}
        <div class="row">
          ...
          <div class="form-group col-md-4">
            <label for="direccion">Nombre Comercial del Stand</label>
            <input type="text" id="nombre_stand" name="nombre_stand" class="form-control" value="{{ null!==(old('nombre_stand')) ? old('nombre_stand') : $sol->nombre_stand}}" placeholder="Marca comercial">
            <strong style="font-size: small">OPCIONAL: Si desea que en el stand salga otro nombre diferente al de la empresa en la cenefa, colóquelo aquí</strong>
          </div>
        </div>
        <div class="row">
          <div class="form-group col-md-4">
    ...
    <div class="container">
      <div class="row justify-content-center mb-5">
        <div class="col-md-7 text-center heading-section ftco-animate fadeInUp ftco-animated">
          <h2 class="mb-1" id="modificar_sucursal">Agregar y Modificar Puntos de Venta</h2>
        </div>
      </div>
      <form class="" id="my_formSUCS" method="POST" action="{{ route('guardar-sucursales') }}">
        {{ csrf_field() }}
        <div class="row">
          <div class="col-md-3">
            <h4>Puntos de Venta actuales</h4>
            Oficina Principal: {{Auth::user()->direccion}} <button type="button" class="btn btn-success" onclick="mostrar_sucursal('0','{{Auth::user()->lat}}','{{Auth::user()->lon}}','{{Auth::user()->direccion}}','','')"><ion-icon name="pencil-sharp"></ion-icon></button><br>
            @foreach ($sucs as $i=>$s)
          ...
          <div class="col-md-5">
            ...
            <div class="form-group" id="telf_visible">
            ...
            <div class="form-group" id="caract_visible">
    ...
    <div class="container">
          ...
          <div class="form-group">
            ...
            <div id="adj_existen" style="display: none">
              ...
              <div class="" id="div_prod_{{$i}}" style="display:none">
                <input type="checkbox" id="check_prod_{{$i}}" onchange="ticks_adj()" name="check_prod[{{$i}}]" checked/>
      ...
      <div class="col-md-4">
        <div class="form-group">
          ...
          <span style="font-size:smaller">Puedes agregar hasta <span id="maxfot">@if($sol->tipo==350) 2 @elseif($sol->tipo==500) 3 @else 5 @endif</span> fotos de cada producto. <strong>El peso máximo por foto es de 2MB</strong></span><br />
    ...
    <div class="col-7">
      <div class="row">
        <div class="form-group col-md-6" >
          <label for="imagen_1" class="control-label">Espacio "A" - Información de la Empresa (medidas: {{$medidas[$sol->tipo][0]}} o superior, peso no mayor a 2MB):</label>
          ...
          @if(file_exists(dirname(base_path()).'/fvirtual/public/img/stand_images/'.Auth::user()->id_empresa.'_1.jpg'))
            <img src="https://virtual.feicobol.com.bo/img/stand_images/{{Auth::user()->id_empresa}}_1.jpg?{{date('Y-m-d H:i:s')}}" style="height:50px">
        ...
        <div class="form-group col-md-6" >
          <label for="imagen_2" class="control-label">Espacio "B" - Productos/Servicios ofertados (medidas: {{$medidas[$sol->tipo][1]}} o superior, peso no mayor a 2MB):</label>
          ...
          @if(file_exists(dirname(base_path()).'/fvirtual/public/img/stand_images/'.Auth::user()->id_empresa.'_2.jpg'))
            <img src="https://virtual.feicobol.com.bo/img/stand_images/{{Auth::user()->id_empresa}}_2.jpg?{{date('Y-m-d H:i:s')}}" style="height:50px">
        ...
        <div class="form-group col-md-6" >
          <label for="imagen_3" class="control-label">Espacio "C" - Información de la Empresa (medidas: {{$medidas[$sol->tipo][2]}} o superior, peso no mayor a 2MB):</label>
          <input type="file" class="form-control" name="imagen_3" accept="image/*" id="imagen_3" />
          @if(file_exists(dirname(base_path()).'/fvirtual/public/img/stand_images/'.Auth::user()->id_empresa.'_3.jpg'))
            <img src="https://virtual.feicobol.com.bo/img/stand_images/{{Auth::user()->id_empresa}}_3.jpg?{{date('Y-m-d H:i:s')}}" style="height:50px">
      ...
      <div class="col-md-7 text-center heading-section ftco-animate fadeInUp ftco-animated">
        <br>
          <h2 class="mb-1" id="encuesta">  Muchas gracias por la información enviada, puede descargar los resultados de su stand aquí<br>
            <a href="informe/{{Auth::user()->id_empresa}}" class="btn btn-primary">Descargar informe del stand</a>
          </h2>
      </div>
    </div>
  </div>
...
@if($encuesta==1)
<script>
const checkboxes = $("[id^='rubro-']");
const checkboxLength = checkboxes.length;
const firstCheckbox = checkboxLength > 0 ? checkboxes.get(0) : null;
function init() {
  if (firstCheckbox) {
    for (let i = 0; i < checkboxLength; i++) {
      if(i<3)
        checkboxes.get(i).addEventListener('change', checkValidity);
      else
      {
        checkboxes.get(3).addEventListener('change',function(){
        if(this.checked)
        {
          $('#otro_rubro_text').prop('required',true);                                
          $('#otro_rubro_text').css('display','block');
        }
        else
        {
          $('#otro_rubro_text').prop('required',false);                                
          $('#otro_rubro_text').css('display','none');
        }
        });
      }
    }
...
function isChecked() {
  for (let i = 0; i < checkboxLength; i++) {
    if (checkboxes.get(i).checked) return true;
  }
  return false;
}
...
function faltante(){
  x=$("[name='informacion_recibida']:checked").val()
  if(x>2)
    {
    $("label[for='informacion_recibida_adic'],#informacion_recibida_adic").css('display','none');
    $("#informacion_recibida_adic").prop('required',false);
    }
  else
  {
    $("label[for='informacion_recibida_adic'],#informacion_recibida_adic").css('display','block');
    $("#informacion_recibida_adic").prop('required',true);
  }
}
function motivos(valor,id){
  if(valor!='')
  {
    if(valor<=0)
        valor=1;
...
function mostrar_sucursal(id,lat,lon,add,caract,fono){
  ...
  $('#caracteristicas_sucursal').val(caract.replace('<br />','\n'));
  $('#telefono_sucursal').val(fono.replace('<br />','\n'));
...
function mostrar_ejecutivo(id,nombre,cargo,modo,link,foto){
  ...
  $('#cargo_ejecutivo').val(cargo.replace('<br />','\n'));
...
function mostrar_prod(id,nombre,envasado,caracteristicas,adj,fotos){
  ...
  $('#caracteristicas_producto').val(caracteristicas.replace('<br />','\n'));
  if(adj!=''){
    $('#adjs').val(adj);
    var adjs=adj.split(";");
    ...
    {
      $('#url_prod_'+ij).attr('href','https://reactivaproductiva.feicobol.com/pdf/'+adjs[ij]);
